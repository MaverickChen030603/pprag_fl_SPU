...
OUTPUT_ROOT = REPO_ROOT / "V7-HP3" / "outputs"
REPORT_ROOT = REPO_ROOT / "实验分析报告" / "V7-HP3"
...
class UpstreamConfig:
    experiment_name: str = "pprag_fl_v7_hp3"
    task_name: str = "num5_dir_a03_imb00_ts0_v7hp2"
    suite_tag: str = "hp2_multihop_hard"
    ...
    use_reader_feedback: bool = False
    reader_feedback_weight: float = 0.0
    reader_feedback_scale: float = 1.0
    reader_feedback_mode: str = "proxy"
    reader_positive_reward: float = 10.0
    reader_negative_reward: float = -5.0
    reader_reward_top_fraction: float = 0.25
    ...
    def to_flgo_option(self) -> Dict:
        ...
        return {
            ...
            "reader_positive_reward": self.reader_positive_reward,
            "reader_negative_reward": self.reader_negative_reward,
            "reader_reward_top_fraction": self.reader_reward_top_fraction,
        }
    ...
    def suite_root(self) -> Path:
        return REPO_ROOT / "V7-HP3" / "outputs" / self.experiment_name / self.suite_tag
    ...
    def output_dir(self) -> Path:
        ...
        reader_tag = f"reader{int(self.use_reader_feedback)}w{str(self.reader_feedback_weight).replace('.', '')}"
        tag = (
            f"{strategy_tag}_k{self.topk_blocks}_w{self.warmup_rounds}_s{self.seed}_{enc_tag}_"
            f"{score_tag}_{budget_tag}_{hist_tag}_{client_tag}_{block_tag}_{hard_tag}_{util_tag}_{reader_tag}"
        )
