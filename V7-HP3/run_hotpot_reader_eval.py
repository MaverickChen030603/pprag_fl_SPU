#!/usr/bin/env python3
"""Reader/generator answer EM/F1 on top retrieved Hotpot contexts for V7-HP3."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fallback-reader-model", default="google/flan-t5-large")
...
def build_prompt(question: str, ranked_docs: List[Dict[str, Any]], topk: int, causal: bool = False) -> str:
    context_parts = []
    ...
    if causal:
        return (
            "You are a HotpotQA reader. Answer with only the short answer.\n"
            f"Question: {question}\nContext: {context}\nAnswer:"
        )
...
def generate_answers(tokenizer, model, prompts: List[str], device: torch.device, batch_size: int, max_input_length: int, max_new_tokens: int, causal: bool = False) -> List[str]:
    outputs: List[str] = []
    model.eval()
    if causal and tokenizer.pad_token_id is None:
        ...
        for start in range(0, len(prompts), batch_size):
            ...
            generated = model.generate(
                **encoded,
                ...
                num_beams=1 if causal else 4,
                do_sample=False,
                early_stopping=not causal,
            ...
            if causal:
                for i, seq in enumerate(generated):
                    ...
                    outputs.append(_clean_causal_answer(tokenizer.decode(new_tokens, skip_special_tokens=True)))
            else:
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        reader_config = AutoConfig.from_pretrained(reader_name, local_files_only=args.local_reader_only, trust_remote_code=True)
        is_causal = bool(getattr(reader_config, "is_decoder", False) and not getattr(reader_config, "is_encoder_decoder", False))
        reader_tokenizer = AutoTokenizer.from_pretrained(reader_name, local_files_only=args.local_reader_only, trust_remote_code=True)
        if is_causal:
            reader_model = AutoModelForCausalLM.from_pretrained(reader_name, local_files_only=args.local_reader_only, trust_remote_code=True, torch_dtype=torch.bfloat16 if device.type == "cuda" else torch.float32).to(device)
        else:
            reader_model = AutoModelForSeq2SeqLM.from_pretrained(reader_name, local_files_only=args.local_reader_only, trust_remote_code=True).to(device)
    except Exception as exc:
        if not args.fallback_reader_model or args.fallback_reader_model == reader_name:
            raise
        print(f"[reader-fallback] failed to load {reader_name}: {exc}; using {args.fallback_reader_model}")
        reader_name = args.fallback_reader_model
        reader_config = AutoConfig.from_pretrained(reader_name, local_files_only=args.local_reader_only, trust_remote_code=True)
        is_causal = bool(getattr(reader_config, "is_decoder", False) and not getattr(reader_config, "is_encoder_decoder", False))
        reader_tokenizer = AutoTokenizer.from_pretrained(reader_name, local_files_only=args.local_reader_only, trust_remote_code=True)
        if is_causal:
    ...
    for qi, (ex, docs, start, end) in enumerate(prepared):
        ...
        prompts.append(build_prompt(ex["question"], ranked, args.retrieval_topk, causal=is_causal))
    ...
    result = {
        ...
        "requested_reader_model": args.reader_model,
