...
DEFAULT_UPSTREAM = REPO_ROOT / "V7-HP3" / "outputs" / "pprag_fl_v7_hp3"
DEFAULT_STRICT = REPO_ROOT / "V7-HP3" / "outputs" / "hp2_strict_eval"
DEFAULT_REPORT = REPO_ROOT / "实验分析报告" / "V7-HP3"
METHOD_ORDER = ["hypernet_v6", "adaptive_v6", "agent_tail_v7hp2", "agent_memory_v7hp2", "agent_oracle_v7hp2"]
...
def load_strict(strict_root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(strict_root.glob("*/hp2_strict_summary.csv")):
...
def render_table(grouped: dict[str, list[dict[str, Any]]]) -> list[str]:
    lines = [
        "| method/profile | n | avg_budget_topk_hp1 | bridge_block_recall_hp1 | early_evidence_recall_hp1 | selection_diversity_hp1 | hp2_multihop_score |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    seen = set(grouped)
    ...
    for method in ordered:
        ...
        lines.append(
            "| {method} | {n} | {budget} | {bridge} | {early} | {div} | {score} |".format(
                ...
                score=stats(metric_values(rows, "hp2_multihop_score")),
            )
        )
    return lines
...
def render_gaps(grouped: dict[str, list[dict[str, Any]]]) -> list[str]:
    metric = "hp2_multihop_score"
    base = best_baseline(grouped, metric)
    if base is None:
        return ["- baseline 数据不足，暂不能计算 agent gap。"]
    lines = []
    for method in ("agent_tail_v7hp2", "agent_memory_v7hp2", "agent_oracle_v7hp2"):
        values = metric_values(grouped.get(method, []), metric)
        if not values:
            continue
...
def main() -> int:
    ...
    out = args.report_dir / f"v7_hp3_auto_analysis_{args.stamp}.md"
    lines: list[str] = []
    lines.append("# V7-HP3 HotpotQA 自动实验分析报告")
    lines.append("")
    ...
    lines.append("- 当前 HP3 strict 指标是选择行为诊断；它不是 Hotpot 官方 answer F1/EM 或 supporting fact F1 的最终替代。")
    lines.append("")
    lines.append("## 结论摘要")
    lines.append("")
    if not rows:
        lines.append("当前尚未发现 HP3 strict eval 结果。实验可能仍在运行，或 strict evaluator 尚未执行。")
    else:
        all_grouped = group(rows)
        lines.extend(render_gaps(all_grouped))
        lines.append("")
        lines.append(f"- HP3 已收集 strict-eval 记录 {len(rows)} 条，平均预算 top-k：{stats(metric_values(rows, 'avg_budget_topk_hp1'))}。")
        lines.append("- 判断正信号时必须同步看预算：若 agent 预算显著高于 baseline，需要以 `hp2_budget_aligned` 为主结论。")
    lines.append("")
    ...
    if rows:
        ...
        for suite in suites:
            ...
            lines.append("")
        ablation = [r for r in rows if r.get("suite") == "hp2_ablation_signal"]
        ...
        lines.append("## 下一步建议")
        lines.append("")
        lines.append("1. 先看 `hp2_budget_aligned`，确认 agent gap 是否在相同 top-k 预算下仍存在。")
        lines.append("2. 若 `hp2_rare_bridge_tail` gap 最大，把 rarity signal 固化进 agent policy，而不是只作为诊断标签。")
        lines.append("3. 若 strict 指标有正信号，再接 Hotpot 官方 answer/supporting-fact 评估，避免只证明选择器会变而没有证明问答收益。")
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    latest = args.report_dir / "v7_hp3_auto_analysis_latest.md"
