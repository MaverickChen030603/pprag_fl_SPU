#!/usr/bin/env python3
"""Write an analysis report for V7-HP3 Hotpot QA/supporting-fact evaluation."""
...
def main() -> int:
    ...
    parser.add_argument("--output-root", type=Path, default=Path("V7-HP3/outputs/hotpot_official_eval"))
    parser.add_argument("--report-dir", type=Path, default=Path("实验分析报告/V7-HP3"))
    ...
    report_path = args.report_dir / f"v7_hp3_hotpot_official_eval_{ts}.md"
    latest_path = args.report_dir / "v7_hp3_hotpot_official_eval_latest.md"

    lines: List[str] = []
    lines.append("# V7-HP3 Hotpot 官方式 QA 与 Supporting-Fact 评估报告")
    lines.append("")
    ...
    lines.append("## 读数口径")
    lines.append("")
    lines.append("本评估接入 Hotpot 风格的 answer EM/F1、supporting-fact EM/F1、joint EM/F1，并额外记录 `answer_access_at_k`、`support_title_recall_at_k` 与 `all_gold_sp_retrieved_at_k`。当前默认使用 V7-HP3 compact Hotpot 派生数据；answer 采用检索可达性启发式，因此判断 agent 是否转化为真实收益时，优先看 answer access、support recall 与 joint 指标，而不是把 answer EM 视作完整 reader/generator 成绩。")
    lines.append("")
    if not rows:
        lines.append("## 当前结论")
        lines.append("")
