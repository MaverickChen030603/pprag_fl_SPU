...
def allocate_client_budget_v6(
    ...
    ratio = float(predicted_budget_ratio if predicted_budget_ratio is not None else 0.5)
    topk = base
    highly_hard = client_hardness >= hard_client_threshold
    highly_useful = utility_ratio >= utility_expand_threshold
    highly_confident = ratio >= adaptive_expand_threshold
    should_expand = highly_hard and highly_useful and highly_confident and hard_client_bonus_topk > 0
    should_shrink = ratio <= adaptive_shrink_threshold and utility_ratio < 1.0
    if not hard_budget_only and should_shrink:
        topk = max(min_topk, base - 1)
    elif not hard_budget_only and not should_expand:
        topk = allocate_client_budget(
            base_topk=base_topk,
            predicted_budget_ratio=predicted_budget_ratio,
            min_topk=min_topk,
            max_topk=max_topk,
            adaptive_scale=min(adaptive_scale, 0.75),
        )
    if should_expand:
        topk = max(topk, base + int(hard_client_bonus_topk))
    return max(min_topk, min(topk, max_topk))
