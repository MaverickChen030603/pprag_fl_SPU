...
def target_from_stats(
    ...
    if score_mode in {"value", "downstream_value"}:
        ...
        for block in block_names:
            block_stats = stats.get(block, {})
            mean_l2 = float(block_stats.get("mean_l2", 0.0))
            if score_mode == "downstream_value":
                max_abs = float(block_stats.get("max_abs", 0.0))
                mean_abs = float(block_stats.get("mean_abs", 0.0))
                hardness_boost = 1.0
                if mean_abs > 0:
                    hardness_boost += min(max_abs / max(mean_abs, 1e-8), 5.0) * 0.1
                mean_l2 *= hardness_boost
            values.append(mean_l2)
        target = torch.tensor(
            values,
            dtype=torch.float32,
        )
...
class DownstreamAwareHyperNetwork(ValueAwareHyperNetwork):
    def __init__(
        self,
        client_num: int,
        block_names: Sequence[str],
        ...
        super().__init__(
            ...
            current_stat_dim=current_stat_dim,
