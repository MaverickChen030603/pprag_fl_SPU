...
try:
    ...
    from .utility_proxy import (
        apply_reader_step_reward,
        apply_reader_step_reward,
        blend_block_utility_with_reader_feedback,
        estimate_block_utility_map,
        estimate_client_downstream_proxy,
        estimate_reader_feedback_map,
    )
except ImportError:
    from explain_analyzer import write_explain_artifacts
    from hardness_estimator import estimate_block_hardness, estimate_client_hardness
...
class Server(BasicServer):
    def initialize(self, *args, **kwargs):
        ...
        self.use_reader_feedback = bool(self.option.get("use_reader_feedback", False))
        self.reader_feedback_weight = float(self.option.get("reader_feedback_weight", 0.0))
        self.reader_feedback_scale = float(self.option.get("reader_feedback_scale", 1.0))
        self.reader_feedback_mode = str(self.option.get("reader_feedback_mode", "proxy"))
        ...
        if self.selection_strategy in {"hypernet_v2", "hypernet_v3", "hypernet_v5", "hypernet_v6", "agent_rule_v7", "agent_bandit_v7", "agent_tail_v7hp2", "agent_memory_v7hp2", "agent_oracle_v7hp2"}:
            hypernet_cls = DownstreamAwareHyperNetwork if self.selection_strategy in {"hypernet_v5", "hypernet_v6", "agent_rule_v7", "agent_bandit_v7", "agent_tail_v7hp2", "agent_memory_v7hp2", "agent_oracle_v7hp2"} else ValueAwareHyperNetwork
    ...
    def unpack(self, packages_received_from_clients):
        ...
        for pkg in packages_received_from_clients:
            ...
            reader_feedback_map = {}
            if self.use_reader_feedback:
                reader_feedback_map = estimate_reader_feedback_map(
                    self.block_names,
                    stats,
                    client_hardness=client_hardness,
                    reader_feedback_scale=self.reader_feedback_scale,
                )
                if self.reader_feedback_mode in {"step", "cliff", "high_contrast"}:
                    reader_feedback_map = apply_reader_step_reward(
                        reader_feedback_map,
                        positive_reward=float(self.option.get("reader_positive_reward", 10.0)),
                        negative_reward=float(self.option.get("reader_negative_reward", -5.0)),
                        top_fraction=float(self.option.get("reader_reward_top_fraction", 0.25)),
                    )
                downstream_utility_map = blend_block_utility_with_reader_feedback(
                    downstream_utility_map,
                    reader_feedback_map,
                    reader_feedback_weight=self.reader_feedback_weight,
                )
    ...
    def _write_run_metadata(self) -> None:
        metadata = {
            ...
            "hard_budget_only": self.hard_budget_only,
            "agent_strategy_mode": self.agent_strategy_mode,
            "use_reader_feedback": self.use_reader_feedback,
            "reader_feedback_weight": self.reader_feedback_weight,
            "reader_feedback_scale": self.reader_feedback_scale,
            "reader_feedback_mode": self.reader_feedback_mode,
            "suite_tag": self.suite_tag,
            "task_name": self.task_name,
            "seed": int(self.option.get("seed", 0)),
