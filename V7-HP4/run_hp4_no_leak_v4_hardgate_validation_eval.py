...
def train_policy_v4(train_examples: list[tuple[dict[str, Any], list[Any]]], out: Path, epochs: int, lr: float, hidden_dim: int, seed: int, target_topk: int) -> dict[str, Any]:
    ...
    for case, docs in train_examples:
        ...
        hard = torch.tensor(V3.hard_negative_mask(case, docs, top_n=5), dtype=torch.bool, device=device)
    ...
    model_path = out / "hp4_no_leak_policy_v4_hardgate.pt"
    ...
    save_json(out / "hp4_no_leak_policy_v4_training_log.json", logs)
    summary = {
    ...
    save_json(out / "hp4_no_leak_policy_v4_training_summary.json", summary)
    return summary
...
def evaluate(examples: list[tuple[dict[str, Any], list[Any]]], reader: Any, model: Any, device: torch.device, top_k: int, alpha: float, top_gate: int, low_scale: float):
    ...
    for case, docs in examples:
        ...
        for mode in ["baseline_uniform", "hp4_no_leak_policy_v4_hardgate"]:
...
def paired(rows: list[dict[str, Any]], metric: str) -> list[float]:
    ...
    return [
        float(m["hp4_no_leak_policy_v4_hardgate"].get(metric, 0.0)) - float(m["baseline_uniform"].get(metric, 0.0))
        for m in by_id.values()
        if "baseline_uniform" in m and "hp4_no_leak_policy_v4_hardgate" in m
    ]


def write_report(path: Path, train: dict[str, Any], summary: dict[str, Any], sig: dict[str, Any], v1: dict[str, Any], v2: dict[str, Any], v3: dict[str, Any], oracle: dict[str, Any]) -> None:
    def g(payload, mode, metric):
        return float(payload.get(mode, {}).get(metric, 0.0)) - float(payload.get("baseline_uniform", {}).get(metric, 0.0))
    lines = ["# V7-HP4 No-Leak V4 Hardgate Policy", "", "## Training", ""]
    ...
    lines += ["", "## Gap Comparison", "", "| metric | v1 | v2 | v3 sparse | v4 hardgate | oracle |", "| --- | ---: | ---: | ---: | ---: | ---: |"]
    for metric in ["support_recall_at_k", "sp_f1", "answer_f1", "joint_f1"]:
        lines.append(f"| {metric} | {g(v1,'hp4_no_leak_policy',metric):+.4f} | {g(v2,'hp4_no_leak_policy_v2',metric):+.4f} | {g(v3,'hp4_no_leak_policy_v3_sparse',metric):+.4f} | {g(summary,'hp4_no_leak_policy_v4_hardgate',metric):+.4f} | {g(oracle,'hp4_soft_agent',metric):+.4f} |")
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    p.add_argument("--output-root", default="V7-HP4/outputs/hp4_no_leak_v4_hardgate_validation")
    ...
    train = train_policy_v4(train_examples, out, args.epochs, args.lr, args.hidden_dim, args.seed, args.target_topk)
    ...
    report_path = Path(args.report_dir) / "v7_hp4_no_leak_v4_hardgate_validation_latest.md"
    write_report(report_path, train, summary, sig, v1, v2s, v3s, oracle)
