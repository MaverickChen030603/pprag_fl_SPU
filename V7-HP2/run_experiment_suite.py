...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V7-HP2 HotpotQA multihop agent-gap experiments.")
    parser.add_argument(
        "--suite",
...
def method_config(base: UpstreamConfig, seed: int, method: str, **overrides) -> UpstreamConfig:
    ...
    if method == "hypernet_v6":
        ...
        common.update(selection_strategy="agent_tail_v7hp2", agent_profile="hp2_reader_tail_agent", hard_client_bonus_topk=1, use_reader_feedback=True, reader_feedback_weight=0.35, reader_feedback_scale=1.25, reader_feedback_mode="proxy")
    elif method == "agent_memory_reader_v7hp2":
        common.update(selection_strategy="agent_memory_v7hp2", agent_profile="hp2_reader_memory_agent", history_window=7, use_reader_feedback=True, reader_feedback_weight=0.35, reader_feedback_scale=1.25, reader_feedback_mode="proxy")
        ...
        raise ValueError(f"Unknown HP2 method: {method}")
...
def hp2_rare_bridge_tail_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
    ...
    for alpha in (0.1, 0.05):
        scenario = replace(
            ...
            task_name=build_task_name(base.num_clients, "DirichletPartitioner", alpha, base.imbalance, base.task_seed, suffix="v7hp2"),
...
def main() -> int:
    ...
    print(f"V7-HP2 suite analysis report written to {report_path}")
    return 0
