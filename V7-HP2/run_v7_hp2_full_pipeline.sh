...
DEV_SAMPLE="${DEV_SAMPLE:-V7-HP2/data/hotpot_dev_stratified_300.json}"
MAX_EVAL="${MAX_EVAL:-300}"

"$PYTHON_BIN" V7-HP2/run_experiment_suite.py \
  --suite hp2_reader_aligned \
  --rounds "$ROUNDS" \
  --seed-list "$SEEDS" \
  --gpu "$GPU" \
  --batch-size "$BATCH_SIZE" \
  --experiment-name pprag_fl_v7_hp2 \
  --rawdata-path "$TRAIN_RAW" \
  --rag-dataset hotpot_qa \
  --rag-hotpot-split validation \
  --rag-hotpot-max-examples "$MAX_EVAL"

"$PYTHON_BIN" V7-HP2/run_all_hotpot_official_eval.py \
  --upstream-root V7-HP2/outputs/pprag_fl_v7_hp2 \
  --output-root V7-HP2/outputs/hotpot_official_fullwiki_dev300 \
...
"$PYTHON_BIN" V7-HP2/run_all_hotpot_reader_eval.py \
  --upstream-root V7-HP2/outputs/pprag_fl_v7_hp2 \
  --output-root V7-HP2/outputs/hotpot_reader_fullwiki_t5small_dev300 \
  --rawdata-path "$DEV_SAMPLE" \
  --suite hp2_reader_aligned \
  --max-examples "$MAX_EVAL" \
  --prefer-official \
...
"$PYTHON_BIN" V7-HP2/write_hp2_reader_alignment_analysis.py
