...
def parse_args():
    p = argparse.ArgumentParser(description="Compute V7-HP2 HotpotQA multihop strict diagnostic metrics from selection traces.")
...
def score_run(run_dir: Path, upstream_root: Path, output_root: Path) -> dict:
    ...
    record = {
        ...
        "hp2_multihop_score": hp2_score,
        "event_count": total_events,
        "run_dir": str(run_dir),
        "output_dir": str(out_dir),
    }
    write_json(out_dir / "hp2_strict_metrics.json", record)
    return record
...
def main() -> None:
    ...
    for meta in sorted(upstream_root.rglob("run_metadata.json")):
        ...
        if not (run_dir / "round_logs.csv").exists():
            continue
        out_file = output_root / run_dir.relative_to(upstream_root) / "hp2_strict_metrics.json"
        if out_file.exists() and not args.force:
            rows.append(json.loads(out_file.read_text(encoding="utf-8")))
        else:
            rows.append(score_run(run_dir, upstream_root, output_root))
    write_csv(output_root / "hp2_strict_summary.csv", rows)
    write_json(output_root / "hp2_strict_summary.json", rows)
    print(f"Processed {len(rows)} HP2 strict diagnostic runs into {output_root}")
