...
BSP_REWARD_MODE = {
    ...
    "agent_bsp_hf_bandit_strict": "strict",
    "agent_bsp_hf_bandit_retrieval": "retrieval",
}
        ...
        if self.strategy in {"agent_rule_v7", "agent_bandit_v7"} or self.strategy in PM_COMPAT_STRATEGIES:
            ...
            if (self.strategy in {"agent_rule_v7", "agent_bandit_v7"} or self.strategy in PM_COMPAT_STRATEGIES) and agent_strategy_mode == "hard_query_focused" and use_coverage_replace and use_early_prior and 0 < budget_topk < len(self.block_names):
                ...
                if fixed_early_slots >= 0:
                    ...
                    if self.strategy == "agent_pm_bandit_slot":
                        ...
                        if self.strategy.startswith("agent_bsp_hf_bandit"):
                            memory_state = True
                            failure_state = True
                            rarity_state = False
                            instability_state = False
                            effective_rarity_score = 0.0
                        else:
            ...
            return SelectionResult(
                ...
                metadata={
                    ...
                    "bsp_memory_state_enabled": float((self.strategy.startswith("agent_bsp_memory_bandit") and self.strategy != "agent_bsp_memory_bandit_no_history_state") or self.strategy.startswith("agent_bsp_hf_bandit")),
                    "bsp_failure_state_enabled": float(bool(use_pm_failure_memory)),
                    "bsp_rarity_state_enabled": float(bool(use_pm_rarity_memory) and not self.strategy.startswith("agent_bsp_hf_bandit")),
                    "bsp_instability_state_enabled": float(bool(use_pm_instability_penalty) and not self.strategy.startswith("agent_bsp_hf_bandit")),
