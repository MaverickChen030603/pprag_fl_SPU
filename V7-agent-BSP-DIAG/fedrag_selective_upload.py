...
class Server(BasicServer):
    def initialize(self, *args, **kwargs):
        ...
        self.selection_strategy = self.option.get("selection_strategy", "hypernet_v6")
        ...
        self.output_dir = ensure_dir(self.option.get("output_dir", "V7/outputs/manual_run"))
        self.score_mode = str(self.option.get("score_mode", "downstream_value"))
        self.budget_mode = str(self.option.get("budget_mode", "fixed"))
        ...
        self.suite_tag = str(self.option.get("suite_tag", "v6"))
        ...
        self.adaptive_expand_threshold = float(self.option.get("adaptive_expand_threshold", 0.62))
        ...
        self.use_early_prior = bool(self.option.get("use_early_prior", True))
        self.use_coverage_replace = bool(self.option.get("use_coverage_replace", True))
        self.use_memory_ema = bool(self.option.get("use_memory_ema", True))
        self.early_coverage_weight = float(self.option.get("early_coverage_weight", 0.0) or 0.0)
        self.use_dynamic_slots = bool(self.option.get("use_dynamic_slots", False))
        self.difficulty_threshold_low = float(self.option.get("difficulty_threshold_low", 0.3) or 0.3)
        self.difficulty_threshold_high = float(self.option.get("difficulty_threshold_high", 0.7) or 0.7)
        self.fixed_early_slots = int(self.option.get("fixed_early_slots", -1) or -1)
        self.use_pm_failure_memory = bool(self.option.get("use_pm_failure_memory", True))
        self.use_pm_rarity_memory = bool(self.option.get("use_pm_rarity_memory", True))
        self.use_pm_instability_penalty = bool(self.option.get("use_pm_instability_penalty", True))
        self.use_bridge_guard = bool(self.option.get("use_bridge_guard", True))
        self.disable_dynamic_hardness = bool(self.option.get("disable_dynamic_hardness", False))
        ...
        if self.selection_strategy in {"hypernet_v2", "hypernet_v3", "hypernet_v5", "hypernet_v6", "agent_rule_v7", "agent_bandit_v7", "agent_pm_dynamic_full", "agent_pm_dynamic_no_memory", "agent_pm_dynamic_no_failure_memory", "agent_pm_dynamic_no_rarity_memory", "agent_pm_dynamic_no_instability_penalty", "agent_pm_dynamic_no_utility_ema", "agent_pm_bandit_slot", "agent_bsp_bandit_strict", "agent_bsp_bandit_retrieval", "agent_bsp_bandit_reader", "agent_bsp_memory_bandit_strict", "agent_bsp_memory_bandit_retrieval", "agent_bsp_memory_bandit_reader", "agent_bsp_memory_bandit_no_failure_state", "agent_bsp_memory_bandit_no_rarity_state", "agent_bsp_memory_bandit_no_instability_state", "agent_bsp_memory_bandit_no_history_state", "agent_bsp_hf_bandit_strict", "agent_bsp_hf_bandit_retrieval", "agent_fixed_slot_0", "agent_fixed_slot_1", "agent_fixed_slot_2", "agent_fixed_slot_3", "agent_dynamic_slot", "agent_dynamic_no_hardness", "agent_dynamic_no_rarity", "agent_dynamic_no_bridge_guard", "agent_tail_v7hp1", "agent_memory_v7hp1", "agent_oracle_v7hp1"}:
            hypernet_cls = DownstreamAwareHyperNetwork if self.selection_strategy in {"hypernet_v5", "hypernet_v6", "agent_rule_v7", "agent_bandit_v7", "agent_pm_dynamic_full", "agent_pm_dynamic_no_memory", "agent_pm_dynamic_no_failure_memory", "agent_pm_dynamic_no_rarity_memory", "agent_pm_dynamic_no_instability_penalty", "agent_pm_dynamic_no_utility_ema", "agent_pm_bandit_slot", "agent_bsp_bandit_strict", "agent_bsp_bandit_retrieval", "agent_bsp_bandit_reader", "agent_bsp_memory_bandit_strict", "agent_bsp_memory_bandit_retrieval", "agent_bsp_memory_bandit_reader", "agent_bsp_memory_bandit_no_failure_state", "agent_bsp_memory_bandit_no_rarity_state", "agent_bsp_memory_bandit_no_instability_state", "agent_bsp_memory_bandit_no_history_state", "agent_bsp_hf_bandit_strict", "agent_bsp_hf_bandit_retrieval", "agent_fixed_slot_0", "agent_fixed_slot_1", "agent_fixed_slot_2", "agent_fixed_slot_3", "agent_dynamic_slot", "agent_dynamic_no_hardness", "agent_dynamic_no_rarity", "agent_dynamic_no_bridge_guard", "agent_tail_v7hp1", "agent_memory_v7hp1", "agent_oracle_v7hp1"} else ValueAwareHyperNetwork
    ...
    def _select_upload_blocks(self, client_id: int):
        ...
        if self.hypernet is not None:
            ...
            if self.budget_mode in {"adaptive", "adaptive_v5", "adaptive_v6"}:
        ...
        if self.use_history_features and history_map is not None:
            ...
            utility_values = [
                float(history_map.get(block_name, {}).get("downstream_utility_ema", 0.0)) for block_name in self.block_names
            ]
    ...
    def _log_round(self, client_updates: Mapping[str, List]) -> None:
        ...
        self.gv.logger.info(
            "V5 round {} strategy={} avg_payload={:.4f} avg_budget_topk={:.2f}".format(
                record["round"],
                record["strategy"],
