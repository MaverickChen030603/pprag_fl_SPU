...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run isolated V7-agent-BSP-DIAG diagnostic experiments.")
    parser.add_argument(
        "--suite",
    ...
    parser.add_argument("--experiment-name", default="pprag_fl_v7agentbspdiag")
...
def method_config(base: UpstreamConfig, seed: int, method: str, **overrides) -> UpstreamConfig:
    ...
    elif method == "agent_bsp_hf_bandit_strict":
        common.update(selection_strategy=method, agent_profile="v7bspdiag_hf_strict", agent_strategy_mode="hard_query_focused", history_window=11, use_dynamic_slots=True, use_early_prior=True, use_coverage_replace=True, use_memory_ema=False, use_pm_failure_memory=True, use_pm_rarity_memory=False, use_pm_instability_penalty=False, use_bridge_guard=True)
    elif method == "agent_bsp_hf_bandit_retrieval":
        common.update(selection_strategy=method, agent_profile="v7bspdiag_hf_retrieval", agent_strategy_mode="hard_query_focused", history_window=11, use_dynamic_slots=True, use_early_prior=True, use_coverage_replace=True, use_memory_ema=False, use_pm_failure_memory=True, use_pm_rarity_memory=False, use_pm_instability_penalty=False, use_bridge_guard=True)
...
def build_suite(args: argparse.Namespace) -> list[UpstreamConfig]:
    ...
    builders = {
        ...
        "v7bspdiag_hf": lambda b, seeds: [method_config(replace(b, suite_tag="v7bspdiag_hf"), seed, method) for seed in seeds for method in ["hypernet_v6", "agent_rule_v7_dynamic", "agent_pm_bandit_slot", "agent_bsp_memory_bandit_retrieval", "agent_bsp_memory_bandit_no_history_state", "agent_bsp_memory_bandit_no_failure_state", "agent_bsp_hf_bandit_strict", "agent_bsp_hf_bandit_retrieval"]],
    }
