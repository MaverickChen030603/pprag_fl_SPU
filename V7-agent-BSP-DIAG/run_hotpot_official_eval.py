...
try:
    from fid_reader import FiDReader
...
def evaluate_run(args: argparse.Namespace, run_dir: Path, examples: list[dict[str, Any]], output_dir: Path) -> dict[str, Any]:
    ...
    reader = None
    if getattr(args, "reader", "heuristic") == "fid":
        if FiDReader is None:
            raise RuntimeError("FiDReader import failed")
        reader = FiDReader(
            model_name_or_path=args.fid_model,
            device=str(device),
            max_input_length=args.fid_max_input_length,
            max_answer_length=args.fid_max_answer_length,
            num_beams=args.fid_num_beams,
    ...
    for qi, (ex, docs, start, end) in enumerate(prepared):
        ...
        if getattr(args, "passage_ordering", "retrieval_score") == "gold_oracle_debug":
            gold_titles_for_order = {str(t) for t, _ in ex.get("supporting_facts", [])}
            ranked = sorted(ranked, key=lambda d: (str(d.get("title", "")) in gold_titles_for_order, d.get("score", 0.0)), reverse=True)
        elif getattr(args, "passage_ordering", "retrieval_score") == "agent_priority":
            # The retriever output is already the agent-selected model state. Keep
            # score order but expose this setting for sensitivity bookkeeping.
            ranked = list(ranked)
        pred_sp = [[d["title"], int(d["sent_id"])] for d in ranked[:args.support_topk]]
        if reader is not None:
            passages = [d["content"] for d in ranked[:args.answer_topk]]
            reader_result = reader.answer(ex["question"], passages, str(ex["answer"]))
            answer_pred = reader_result["pred_answer"]
            answer_info = {"mode": "fid", "used_passage_ids": reader_result.get("used_passage_ids", [])}
            answer_em = float(reader_result.get("answer_em", exact_match_score(answer_pred, ex["answer"])))
            answer_f1 = float(reader_result.get("answer_f1", f1_score(answer_pred, ex["answer"])[0]))
            answer_prec = answer_rec = answer_f1
        else:
        ...
        rec.update({"example_id": ex["id"], **rec["metrics"]})
    ...
    result = {
        ...
        "metrics": means,
        "reader": getattr(args, "reader", "heuristic"),
        "fid_model": getattr(args, "fid_model", ""),
        ...
        "avg_budget_topk": metadata.get("avg_budget_topk") or (metadata.get("option") or {}).get("selective_topk_blocks") or 3.0,
        "budget_std": metadata.get("budget_std", 0.0),
        "reader_model": getattr(args, "fid_model", ""),
        "beam_size": int(getattr(args, "fid_num_beams", 0)),
        "max_input_length": int(getattr(args, "fid_max_input_length", 0)),
        "max_output_length": int(getattr(args, "fid_max_answer_length", 0)),
        "passage_ordering": getattr(args, "passage_ordering", "retrieval_score"),
    }
    (output_dir / "official_metrics.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reader", choices=["heuristic", "fid"], default="heuristic")
    parser.add_argument("--fid-model", default="t5-base")
    parser.add_argument("--fid-max-input-length", type=int, default=512)
    parser.add_argument("--fid-max-answer-length", type=int, default=32)
    parser.add_argument("--fid-num-beams", type=int, default=4)
    parser.add_argument("--passage-ordering", choices=["retrieval_score", "agent_priority", "gold_oracle_debug"], default="retrieval_score")
