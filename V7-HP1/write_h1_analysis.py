#!/usr/bin/env python3
"""Write a compact Chinese analysis report for V7-HP1."""
...
DEFAULT_STRICT = REPO_ROOT / "V7-HP1" / "outputs" / "h1_strict_eval"
...
def main() -> int:
    ...
    if rows:
        ...
        lines.append("## 下一步建议")
        lines.append("")
        lines.append("1. 若 `agent_oracle_v7hp1` 有明显正 gap 而 fair agent 没有，优先学习 oracle 的 block/action pattern，而不是继续调评估。")
        lines.append("2. 若 `agent_tail_v7hp1` 只在 rare-domain suite 有正 gap，应把 rarity/hard-client 信号接入更细粒度的 block-value predictor。")
