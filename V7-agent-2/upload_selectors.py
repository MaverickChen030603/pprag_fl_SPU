...
class UploadSelector:
    ...
    def select(
        ...
        if self.strategy in {"agent_rule_v7", "agent_bandit_v7"}:
            ...
            if self.strategy in {"agent_rule_v7", "agent_bandit_v7"} and agent_strategy_mode == "hard_query_focused" and use_coverage_replace and use_early_prior and 0 < budget_topk < len(self.block_names):
                ...
                if use_dynamic_slots:
                    difficulty = max(0.0, min(1.0, float(client_hardness or 0.5)))
                    early_slots = get_dynamic_early_slots(difficulty, budget_topk, difficulty_threshold_low, difficulty_threshold_high)
                else:
                    early_slots = 1
