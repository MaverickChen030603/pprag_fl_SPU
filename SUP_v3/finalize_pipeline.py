...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Finalize the SUP_v3 upstream + downstream experiment pipeline.")
    parser.add_argument("--suite-name", default="all")
    parser.add_argument("--wait-pid", type=int, default=0)
    parser.add_argument("--poll-seconds", type=int, default=120)
    parser.add_argument("--upstream-root", default=str(OUTPUT_ROOT / "pprag_fl_sup_v3" / "adhoc"))
    parser.add_argument("--downstream-root", default=str(OUTPUT_ROOT / "rag_eval_all_v2" / "adhoc"))
