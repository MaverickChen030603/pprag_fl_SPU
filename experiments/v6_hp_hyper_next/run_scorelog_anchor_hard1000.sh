...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-v6_hp_hyper_next_scorelog_anchor_$STAMP}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT_DIR/experiments/v6_hp_hyper_next/results/scorelog_anchor_hard1000_$STAMP}"
RAW_CSV="${RAW_CSV:-$ROOT_DIR/experiments/v6_hp_hyper_next/results/scorelog_anchor_hard1000_raw.csv}"
COMMAND_LOG="$ROOT_DIR/experiments/v6_hp_hyper_next/logs/scorelog_anchor_hard1000_commands.log"
...
latest_run_dir() {
  local version_dir="$1"
  local suite_tag="$2"
  find "$version_dir/outputs/$EXPERIMENT_NAME/$suite_tag" -name run_metadata.json -print 2>/dev/null | sort | tail -n 1 | xargs dirname
}
...
run_case() {
  ...
  log_msg "[scorelog-anchor-start] $(date '+%F %T %Z') method=$method"
  "$PYTHON_BIN" "$script" \
    --strategy "$strategy" \
    --topk 2 \
    --warmup 0 \
  ...
  log_msg "[scorelog-anchor-end] $(date '+%F %T %Z') method=$method run_dir=$run_dir"
}

log_msg "[scorelog-anchor-suite-start] $(date '+%F %T %Z') experiment=$EXPERIMENT_NAME subset=$SUBSET_NAME"
...
log_msg "[scorelog-anchor-suite-done] $(date '+%F %T %Z') raw_csv=$RAW_CSV output_root=$OUTPUT_ROOT"
