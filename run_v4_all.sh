...
UPSTREAM_ROOT="$ROOT_DIR/V4/outputs/pprag_fl_v4"
DOWNSTREAM_ROOT="$ROOT_DIR/V4/outputs/rag_eval_all_v4"
REPORT_ROOT="$ROOT_DIR/实验分析报告/V4"
...
ensure_finalize() {
  ...
  local cmd=(
    "$PYTHON_BIN" V4/finalize_pipeline.py
...
run_suite() {
  ...
  if ls "$REPORT_ROOT"/suite_"$suite_name"_* >/dev/null 2>&1; then
    ...
    echo "[run] $suite_name"
    "$PYTHON_BIN" V4/run_experiment_suite.py \
      --suite "$suite_name" \
      --gpu "$GPU_ID" \
...
run_suite v4_main
run_suite v4_budget
run_suite v4_heterogeneity
run_suite v4_hardquery
run_suite v4_ablation_signal
run_suite v4_ablation_budget
run_suite v4_explain

if ls "$REPORT_ROOT"/full_pipeline_all_v4_* >/dev/null 2>&1; then
  echo "[skip] full_pipeline_all_v4 already exists"
else
  cmd=(
    "$PYTHON_BIN" V4/finalize_pipeline.py
    --suite-name all_v4
  ...
  echo "[finalize] all_v4"
  "${cmd[@]}"
fi

echo "run_v4_all completed"
