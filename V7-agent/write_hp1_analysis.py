...
DEFAULT_UPSTREAM = REPO_ROOT / "V7-agent" / "outputs" / "pprag_fl_v7_agent"
DEFAULT_STRICT = REPO_ROOT / "V7-agent" / "outputs" / "hp1_strict_eval"
DEFAULT_REPORT = REPO_ROOT / "实验分析报告" / "V7-agent"
...
def main() -> int:
    ...
    out = args.report_dir / f"v7_agent_auto_analysis_{args.stamp}.md"
    lines: list[str] = []
    lines.append("# V7-agent HotpotQA Agent-Rule 自动实验分析报告")
    lines.append("")
    ...
    lines.append("- 当前 V7-agent/HP1 strict 指标是选择行为诊断；它不是 Hotpot 官方 answer F1/EM 或 supporting fact F1 的最终替代。")
    lines.append("")
    lines.append("## 结论摘要")
    lines.append("")
    if not rows:
        lines.append("当前尚未发现 V7-agent strict eval 结果。实验可能仍在运行，或 strict evaluator 尚未执行。")
    else:
        all_grouped = group(rows)
        lines.extend(render_gaps(all_grouped))
        lines.append("")
        lines.append(f"- V7-agent 已收集 strict-eval 记录 {len(rows)} 条，平均预算 top-k：{stats(metric_values(rows, 'avg_budget_topk_hp1'))}。")
    ...
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    latest = args.report_dir / "v7_agent_auto_analysis_latest.md"
