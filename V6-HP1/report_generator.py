...
从方法行为看，本次策略为 `{report['strategy']}`。如果采用的是 `hypernet_v6`、`hypernet_v5` 或 `hypernet_v3`，则需要同时关注 `score_mode={report['score_mode']}` 与 `budget_mode={report['budget_mode']}`：前者决定当前是按重要性、价值密度还是下游代理价值排序，后者决定预算是否会随客户端状态自适应变化。平均超网络损失为 {report['avg_hn_loss']:.4f}，单位通信收益代理为 {report['utility_per_payload']:.4f}，它们可以作为 V6-HP1 是否真正学到“在 Hotpot same-budget 条件下更值的参数块”的辅助观察信号。
    ...
    if report.get("runs"):
        ...
        lines.extend(
            [
                ...
                "如果 `hypernet_v6` 在保持相近 payload ratio 的同时还能带来更高的 `utility_per_payload` 或更好的下游指标，说明 V6-HP1 在 Hotpot same-budget 设定下开始发挥作用；如果固定预算下已经优于自适应扩预算版本，则说明其固定预算内替换策略是有效的。",
            ]
        )
    else:
