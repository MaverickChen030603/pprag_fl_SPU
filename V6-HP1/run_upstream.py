...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V6-HP1 upstream FL experiment.")
    parser.add_argument(
        "--strategy",
        default="hypernet_v6",
        choices=["full", "random", "static_top", "delta_norm", "hypernet_v2", "hypernet_v3", "hypernet_v5", "hypernet_v6"],
    )
    ...
    parser.add_argument("--suite-tag", default="v6hp1_adhoc")
...
def ensure_task(config: UpstreamConfig) -> None:
    ...
    try:
        flgo.gen_task(
            ...
            rawdata_path=str(Path(config.rawdata_path).expanduser().resolve()),
...
def main() -> None:
    ...
    print(f"V6-HP1 analysis report written to {report_path}")
