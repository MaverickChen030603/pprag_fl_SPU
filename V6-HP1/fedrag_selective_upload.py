...
def _build_score_log_record(
    *,
    ...
    ranked_non_selected = sorted(non_selected_scores, reverse=True)
    selected_min = min(selected_scores) if selected_scores else None
    best_non_selected = ranked_non_selected[0] if ranked_non_selected else None
    ...
    return {
        ...
        "selection_metadata": dict(metadata or {}),
    }
...
class Server(BasicServer):
    def initialize(self, *args, **kwargs):
        ...
        self.output_dir = ensure_dir(self.option.get("output_dir", "V6/outputs/manual_run"))
        ...
        self.pooler_cap_ratio = self.option.get("pooler_cap_ratio", None)
        self.pooler_cap_ratio = None if self.pooler_cap_ratio in {None, "None", ""} else float(self.pooler_cap_ratio)
        self.exclude_pooler = bool(self.option.get("exclude_pooler", False))
        ...
        if self.selection_strategy in {"hypernet_v2", "hypernet_v3", "hypernet_v5", "hypernet_v6"}:
            hypernet_cls = DownstreamAwareHyperNetwork if self.selection_strategy in {"hypernet_v5", "hypernet_v6"} else ValueAwareHyperNetwork
