...
from search_space_stage2 import Stage2TrialConfig, suggest_stage2_trial_config
...
def _command_for_trial(*, python_bin: str, trial_number: int, cfg: Stage2TrialConfig, args: Any) -> list[str]:
...
def _run_rag_eval(*, python_bin: str, model_dir: Path, output_dir: Path, args: Any) -> None:
    command = [
        ...
        args.hotpot_split,
        "--eval-subset-type",
        args.eval_subset_type,
        "--eval-num-examples",
        str(args.eval_num_examples),
    ]
    if args.hard_query_subset:
        command.extend(["--hard-query-subset", str(Path(args.hard_query_subset).expanduser().resolve())])
...
def objective_factory(args: Any):
    ...
    def objective(trial: Any) -> float:
        cfg = suggest_stage2_trial_config(
            trial,
            payload_penalty_mode=args.payload_penalty_mode,
            fixed_payload_penalty=args.payload_penalty,
        )
        ...
        score = compute_objective(metrics, payload, cfg.payload_penalty)
        ...
        result = {
            ...
            "metrics": metrics,
            "payload_penalty": cfg.payload_penalty,
        ...
        trial.set_user_attr("payload_penalty", cfg.payload_penalty)
