...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run V6-H1 hard-query experiment suites.")
    parser.add_argument(
        "--suite",
        default="v6h1_main",
        choices=[
    ...
    parser.add_argument("--experiment-name", default="pprag_fl_v6_h1")
...
def v6h1_main_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
...
def v6h1_budget_aligned_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
    configs: list[UpstreamConfig] = []
    method_grid = [
...
def v6h1_heterogeneity_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
    configs: list[UpstreamConfig] = []
...
def v6h1_hardquery_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
    configs: list[UpstreamConfig] = []
    for seed in seeds:
        configs.extend(
            [
...
def v6h1_ablation_signal_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
    configs: list[UpstreamConfig] = []
    for seed in seeds:
        configs.extend(
            [
...
def v6h1_ablation_budget_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
    configs: list[UpstreamConfig] = []
    for seed in seeds:
        configs.extend(
            [
...
def v6h1_explain_suite(base: UpstreamConfig, seeds: Iterable[int]) -> list[UpstreamConfig]:
    return [
        replace(base, seed=seed, selection_strategy="hypernet_v6", topk_blocks=3, warmup_rounds=1, score_mode="downstream_value", budget_mode="fixed", layerwise_budget=True)
        for seed in seeds
    ]
...
def build_suite(args: argparse.Namespace) -> list[UpstreamConfig]:
    ...
    if args.suite == "all_v6h1":
        suite_configs.extend(v6h1_main_suite(replace(base, suite_tag="v6h1_main"), seeds))
        suite_configs.extend(v6h1_budget_aligned_suite(replace(base, suite_tag="v6h1_budget_aligned"), seeds))
        suite_configs.extend(v6h1_heterogeneity_suite(replace(base, suite_tag="v6h1_heterogeneity"), seeds))
        suite_configs.extend(v6h1_hardquery_suite(replace(base, suite_tag="v6h1_hardquery"), seeds))
        suite_configs.extend(v6h1_ablation_signal_suite(replace(base, suite_tag="v6h1_ablation_signal"), seeds))
        suite_configs.extend(v6h1_ablation_budget_suite(replace(base, suite_tag="v6h1_ablation_budget"), seeds))
        suite_configs.extend(v6h1_explain_suite(replace(base, suite_tag="v6h1_explain"), seeds))
    else:
        builders = {
            "smoke": smoke_suite,
            "v6h1_main": v6h1_main_suite,
            "v6h1_budget_aligned": v6h1_budget_aligned_suite,
            "v6h1_heterogeneity": v6h1_heterogeneity_suite,
            "v6h1_hardquery": v6h1_hardquery_suite,
            "v6h1_ablation_signal": v6h1_ablation_signal_suite,
            "v6h1_ablation_budget": v6h1_ablation_budget_suite,
            "v6h1_explain": v6h1_explain_suite,
        }
...
def main() -> int:
    ...
    for index, config in enumerate(suite_configs, start=1):
        ...
        run(config)
        results.append({"status": "completed", "output_dir": str(config.output_dir), "config": config.to_dict()})

    write_json(manifest_root / "suite_results.json", results)
    report_path = write_suite_report(args.suite, suite_configs)
    print(f"V6-H1 suite analysis report written to {report_path}")
    return 0
