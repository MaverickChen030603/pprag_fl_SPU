...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run downstream RAG evaluation for every upstream V6-H1 run.")
    parser.add_argument("--upstream-root", default=str(OUTPUT_ROOT / "pprag_fl_v6_h1"))
    parser.add_argument("--output-root", default=str(OUTPUT_ROOT / "rag_eval_all_v6_h1"))
    ...
    parser.add_argument("--save-per-query", action="store_true", help="Write per-query JSONL files beside each downstream log.")
...
def discover_runs(root: Path, include_pattern: str) -> list[Path]:
    ...
    if not root.exists():
        return runs
    candidates = sorted(path.parent for path in root.rglob("run_metadata.json"))
...
def run_eval(args: argparse.Namespace, run_dir: Path) -> dict:
    ...
    relative_run_dir = run_dir.relative_to(upstream_root)
    output_dir = ensure_dir(Path(args.output_root) / relative_run_dir)
    ...
    for run_dir in discover_runs(upstream_root, args.include_pattern):
        try:
            ...
            records.append(
                {
                    "run_dir": str(run_dir),
                    "relative_run_dir": str(run_dir.relative_to(upstream_root)),
                    "status": "failed",
