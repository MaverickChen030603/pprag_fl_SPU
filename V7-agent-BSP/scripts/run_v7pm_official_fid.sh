...
' | sort)
echo "[$(date '+%F %T')] START official FiD/T5 runs=${#RUNS[@]} examples=$MAX_EXAMPLES device=$DEVICE" | tee -a "$LOG"
idx=0
for RUN in "${RUNS[@]}"; do
  idx=$((idx+1))
  REL="${RUN#$UPSTREAM/}"
  ...
  "$PYTHON_BIN" "$BASE/run_hotpot_official_eval.py" --run-dir "$RUN" --rawdata-path "$RAWDATA" --output-dir "$OUT"     --max-examples "$MAX_EXAMPLES" --support-topk 2 --answer-topk 5 --batch-size "$BATCH_SIZE"     --device "$DEVICE" --reader fid --fid-model "$FID_MODEL" 2>&1 | tee -a "$LOG"
done
