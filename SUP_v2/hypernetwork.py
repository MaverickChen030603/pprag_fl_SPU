...
def infer_bert_block_name(param_name: str) -> str:
    """Map BERT/BGE-style parameter names to communication blocks."""
...
def build_block_map(
    ...
    """Build a block -> parameter names map for payload accounting."""
...
def stats_to_features(
    ...
    l2 = torch.tensor([float(stats.get(block, {}).get("l2", 0.0)) for block in block_names])
    mean_abs = torch.tensor([float(stats.get(block, {}).get("mean_abs", 0.0)) for block in block_names])
    max_abs = torch.tensor([float(stats.get(block, {}).get("max_abs", 0.0)) for block in block_names])
    return torch.stack([normalize_vector(l2), normalize_vector(mean_abs), normalize_vector(max_abs)], dim=-1)
...
def target_from_stats(
    ...
    target = torch.tensor([float(stats.get(block, {}).get("l2", 0.0)) for block in block_names])
    return normalize_vector(target)


class BlockImportanceHyperNetwork(nn.Module):
    """Hypernetwork that predicts client-specific block upload importance."""
    ...
    def forward(
        ...
        features = torch.cat(
            [self.client_embedding(client_ids), self.block_embedding(block_ids), stat_features],
            dim=-1,
        )
