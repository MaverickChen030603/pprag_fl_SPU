...
def main():
 ...
 passes=[x for x in trainable if float(x['top5_set_changed_rate'])>=.05 and (float(x['support_rank_changed_rate'])>=.10 or float(x['complete_support_status_changed_rate'])>=.03 or float(x['top10_set_changed_rate'])>=.10) and fwd.get(x['condition'],False)]
