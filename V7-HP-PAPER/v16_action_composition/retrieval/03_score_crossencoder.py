#!/usr/bin/env python3
"""Attach matched CrossEncoder scores to a frozen V16 pool."""
...
def main() -> None:
    ...
    with args.pool.open(encoding="utf-8") as source, args.output.open("w", encoding="utf-8") as target:
        for line in source:
            ...
            row = json.loads(line)
            docs = row["pool"]
            scores = model.predict([(questions[str(row["query_id"])], f"{doc['title']}. {doc['text']}") for doc in docs], batch_size=args.batch_size, show_progress_bar=False)
    ...
    print(json.dumps({"status": "complete", "output": str(args.output), "checkpoint": args.checkpoint}, indent=2))
