#!/usr/bin/env python3
"""Validate V16 split exclusivity, exclusion, hashes, labels, and permissions."""
...
FORBIDDEN_TOP_LEVEL = {"answer", "answers", "answer_aliases", "gold_answer", "supporting_facts", "supporting_titles", "evidence", "evidences", "supporting_paragraphs", "reasoning_path", "question_decomposition", "answerable"}
...
def main() -> None:
    ...
    for dataset, record in manifest["datasets"].items():
        ...
        used_questions = set(inventory.get("used_normalized_questions_by_dataset", {}).get(dataset, [])) | set(inventory.get("used_normalized_questions_by_dataset", {}).get("unknown", []))
        ...
        for split, file_record in record["files"].items():
            ...
            if split == "final_test":
                for index, row in enumerate(rows):
                    ...
                    if top or nested:
                        failures.append(f"{dataset}/final_test row {index}: label leakage top={sorted(top)} nested={nested[:3]}")
                        break
        ...
        datasets[dataset] = {"counts": {name: len(values) for name, values in split_ids.items()}, "strata": record["strata"], "used_id_overlap": 0, "used_question_overlap": 0}
    payload = {"status": "pass" if not failures else "fail", "datasets": datasets, "failures": failures}
    (PROTOCOL / "frozen_split_audit.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
