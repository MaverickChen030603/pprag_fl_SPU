...
从方法行为看，本次策略为 `{report['strategy']}`。如果采用的是 `hypernet_v3`，则需要同时关注 `score_mode={report['score_mode']}` 与 `budget_mode={report['budget_mode']}`：前者决定当前是按重要性还是按价值密度排序，后者决定预算是否会随客户端状态自适应变化。平均超网络损失为 {report['avg_hn_loss']:.4f}，单位通信收益代理为 {report['utility_per_payload']:.4f}，它们可以作为 V3 是否真正学到“更值的参数块”的辅助观察信号。
    ...
    if report.get("runs"):
        ...
        lines.extend(
            [
                ...
                "如果 `hypernet_v3` 在保持较低 payload ratio 的同时还能带来更高的 `utility_per_payload`，说明价值感知排序开始发挥作用；如果 `budget_mode=adaptive` 相对固定预算更优，则说明客户端自适应预算设计是有效的。",
            ]
        )
    else:
